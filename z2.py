...
def status_eh_ignorado_por_raw(status_cru: str, ignored_set: set = None) -> bool:
    """
    Retorna True se o status (cru) estiver na lista de ignorados, considerando normalização.
    Usa o set pré-normalizado (IGNORED_STATUSES_NORM) se não for passado outro.
    """
    if not status_cru:
        return False
...
def preparar_updates_por_cnpj(
    ...
    """
    Versão corrigida:
      - IGNORED_STATUSES (eventos): filtra eventos do NEO após normalização.
      - IGNORED_STATUS_GSHEETS_NORM (planilha): se a linha já tem um STATUS nessa lista, não atualiza a linha.
      - STATUS_EQUIVALENCE aplicado antes de qualquer decisão/escrita.
      - Regras TRANSF > RENOVAÇÃO, e 'ATIVADO 100%' conta dupla para não-TRANSF.
      - Preenche múltiplas colunas de data via STATUS_DATE_LISTS (info['date_cols_idx']).
      - Preenche COTAÇÃO (numeroPedidoVinculado) sem sobrescrever.
    """
    updates = []
    updated_rows = []
    ...
    for k, v in raw_status_equiv.items():
        try:
            # chave: o QUE vier do NEO (cru), mas vamos comparar usando normalize_for_compare
            k_norm = normalize_for_compare(k)
            # valor: como queremos escrever / comparar internamente
    ...
    def map_status_for_logic(etapa_raw: str) -> str:
        """Normaliza o status do NEO e aplica equivalência (se existir)."""
    ...
    for row_idx, linha in enumerate(todas_linhas[1:], start=2):
        ...
        # >>> AQUI: bloqueio de LINHAS por IGNORED_STATUS_GSHEETS_NORM (somente planilha)
    ...
    for ev in eventos:
        ...
        if not etapa_raw:
            continue
        # >>> AQUI: bloqueio de EVENTOS por IGNORED_STATUSES
    ...
    if transf_present:
        for ev in eventos_ordenados:
            ...
            if TRANSF_PROD_NORM in produtos_alvo and map_status_for_logic(etapa_raw) == ACTIVATE_NORM:
                transf_activated = True
                break

    # --- Regra TRANSF (prioridade máxima): força "AGUARDANDO CONCLUIR TT" até ativar ---
    ...
    # --- helper: eventos aplicáveis à linha (com status já mapeado) ---
    ...
    # --- Processar linha a linha (após TRANSF aplicado) ---
    for linha in rows:
        # Se TRANSF está presente e ainda não ativado, todas as outras linhas já foram fixadas em AGUARDANDO -> não mexe
        ...
        # Regra RENOVAÇÃO (prioridade após TRANSF)
        if tipo_comp == normalize_for_compare("RENOVAÇÃO"):
            # Se o histórico do CNPJ contém "CONCLUÍDO INSPEÇÃO" em algum momento,
            # e NUNCA teve "ATIVADO 100%" ainda, e a linha NÃO tem chip preenchido,
            # então travamos RENOVAÇÃO em "CONCLUÍDO INSPEÇAO" (independente do último evento)
        ...
        # Aplicar eventos específicos da linha (mapeados)
        ...
        for ev, status_mapped in evs_linha:
            if status_mapped == ACTIVATE_NORM:
                ...
                if linha.get("produto_comp") == TRANSF_PROD_NORM:
                    # TRANSF ativa na 1ª vez
                    candidate = status_mapped
                else:
                    # não-TRANSF: exige 2 ocorrências
                    if ativado_count >= 2:
                        candidate = status_mapped
            else:
                candidate = status_mapped
    ...
    # --- Log quando TRANSF finalmente ativa ---
    if transf_present:
